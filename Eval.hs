...
evalExpr' ce (Div e1 e2) g =  do (t1,v1) <- evalExpr' ce e1 g
			         (t2,v2) <- evalExpr' ce e2 g
			         if (eqTypes t1 TNumeric && eqTypes t2 TNumeric) then 
...
evalExpr' ce (Menor e1 e2) g =do (t1,v1) <- evalExpr' ce  e1 g
			         (t2,v2) <- evalExpr' ce e2 g
			         if (eqTypes t1 TNumeric && eqTypes t2 TNumeric) then return (TBoolean, boolean (num v1 < num v2)) else raise (Err "VALOR")
...
evalExpr' ce (And e1 e2) g = do (t1,v1) <- evalExpr' ce e1 g
				(t2,v2) <- evalExpr' ce e2 g
				if (eqTypes t1 TBoolean && eqTypes t2 TBoolean) then funcBoolean (\x y -> x && y) v1 v2 else raise (Err "VALOR")
...
evalExpr' ce (Abs e) g = do (t,v) <- evalExpr' ce e g
		            if (eqTypes t TNumeric) then funcUnNumeric (\x -> abs(x)) v  else raise (Err "VALOR")
...
evalExpr' ce (Opuesto e) g = do (t,v) <- evalExpr' ce e g
			        if (eqTypes t TNumeric) then funcUnNumeric (\x -> -1*x) v else raise (Err "VALOR")
